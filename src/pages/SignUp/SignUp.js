...
const SignUp = () => {
  ...
  const handleSignInWithGoogle = () => {
    signInWithGoogle()
      .then((result) => {
        ...
        saveUserInfo(user.displayName, user.email, usertype);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const saveUserInfo = (name, email, usertype) => {
    ...
    fetch("https://bike-hunter-server.vercel.app/users", {
      ...
      .then((data) => {
        console.log(data);
        fetch(`https://bike-hunter-server.vercel.app/jwt?email=${email}`)
          .then((res) => res.json())
          .then((data) => {
            if (data.accessToken) {
    ...
    <div className="h-[800px] flex justify-center items-center">
      <div className="w-96 p-7">
        ...
        <form onSubmit={handleSubmit(handleSignUp)}>
          ...
          <label className="label">
            {" "}
            <span className="label-text">Pick the type of user.</span>
          </label>
          <select
            className="select w-full max-w-xs form-control input-bordered"
            required
            {...register("user")}
          >
